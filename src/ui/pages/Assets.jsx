...
export default function Assets({state, setState}){
  ...
  const totalReserveDisplay = useMemo(()=>{
    return state.assets.filter(a=>a.verified).reduce((sum,a)=> sum + (a.history.length ? a.history[a.history.length-1].display : a.declaredValueEUR),0)
  },[state.assets])
  ...
  function runDailyValuation(epochISO){
    ...
    let updatedAssets = state.assets.map(a=>{
      ...
      const past = a.history.filter(h=>h.date<iso).sort((a,b)=>a.date<b.date?1:-1).slice(0,2).map(h=>h.value)
    ...
    const totalReserve = updatedAssets.filter(a=>a.verified).reduce((s,a)=> s + (a.history.length? a.history[a.history.length-1].display : a.declaredValueEUR),0)
    ...
    setState(s=>({ ...s, assets: updatedAssets, token:{...s.token, navPerToken}, oracle:[...s.oracle.filter(o=>o.epoch!==iso), newSnap], lastEpoch: iso }))
  }

  function toggleVerify(id){
    setState(s=>({ ...s, assets: s.assets.map(a=> a.id===id ? { ...a, verified: !a.verified } : a) }))
  }
  ...
  function mintForAsset(id){
    ...
    const lp = V*0.1
    setState(s=>({ ...s, token:{ totalSupply: s.token.totalSupply+mintAmount, navPerToken: (s.oracle[s.oracle.length-1]?.totalReserveValue||0)/(s.token.totalSupply+mintAmount||1), liquidityFundEUR: s.token.liquidityFundEUR+lp } }))
  }
  ...
  const filtered = useMemo(()=>{
    return state.assets.filter(a=>{
      ...
      const hit = !q || [a.brand, a.model, a.title, a.artist, a.serial].filter(Boolean).join(' ').toLowerCase().includes(text)
      return inCat && hit
    })
  },[state.assets, q, cat])

  const [draft, setDraft] = useState({ category:'watch', brand:'', model:'', serial:'', title:'', artist:'', year:'', condition:'', declaredValueEUR:'' })
  function fingerprintFromDraft(){
    return ['category','brand','model','serial','title','artist','year','condition'].map(k=> (draft[k]||'').trim().toLowerCase()).join('|')
  }
  function addAsset(){
    const declared = Number(draft.declaredValueEUR||0); if(!declared) return alert('Valore dichiarato?')
    const fp = 'k'+(Math.abs(fingerprintFromDraft().split('').reduce((a,c)=>a+c.charCodeAt(0),0))).toString(16)
    if(state.assets.find(a=>a.fingerprint===fp)) return alert('Asset già presente')
    const id = 'A'+Date.now()
    const base = { id, category: draft.category, brand: draft.brand||undefined, model: draft.model||undefined, serial: draft.serial||undefined, title:draft.title||undefined, artist:draft.artist||undefined, year:draft.year||undefined, condition:draft.condition||undefined, declaredValueEUR: declared, fingerprint: fp, verified:false, createdAt: todayISO(), history: [] }
    ...
    <main className="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
      <div className="lg:col-span-4 space-y-6">
        <Card>
          ...
          <div className="mt-3"><button onClick={()=>runDailyValuation()} className="px-3 py-2 rounded-xl border bg-white hover:bg-slate-50 text-sm">Run valutazione</button></div>
        </Card>
        <Card>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <button onClick={addAsset} className="col-span-2 px-3 py-2 rounded-xl border bg-black text-white">Aggiungi</button>
          </div>
        </Card>
      </div>
      <div className="lg:col-span-8 space-y-6">
        <Card className="p-0">
          <div className="p-5 flex items-center justify-between">
            ...
            <div className="text-xs text-slate-500">Epoch: <span className="font-mono">{state.lastEpoch || todayISO()}</span></div>
          </div>
          <div className="px-5 pb-5 grid md:grid-cols-2 gap-4">
            ...
            {filtered.map(a=>{
              ...
              return (
                <Link to={`/asset/${a.id}`} key={a.id} className="rounded-2xl border bg-white p-4 hover:bg-slate-50">
                  <div className="text-[10px] uppercase tracking-wide text-slate-500">{CATEGORIES.find(c=>c.id===a.category)?.label}</div>
                  <div className="font-semibold">{[a.artist||a.brand, a.title||a.model].filter(Boolean).join(' — ')||a.id}</div>
                  {a.serial && <div className="text-xs text-slate-500">Seriale: {a.serial}</div>}
                  <div className="mt-2 text-sm">Valore: <strong>{fmtEUR(display)}</strong></div>
                  <div className="mt-1 text-xs">{a.verified? 'Verified' : 'Pending check'}</div>
                </Link>
              )
            })}
          </div>
