...
function InfoTip({ text }) {
  ...
  return (
    <span className="relative inline-block align-middle">
      ...
      {open && (
        <div className="absolute z-20 mt-2 w-64 p-2 text-xs bg-white border rounded-lg shadow">
          {text}
        </div>
      )}
    </span>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-2xl bg-white rounded-2xl border shadow-lg"
      >
...
export default function Home({ state }) {
  ...
  function seedFrom(str) {
    let n = 0
    for (let i=0;i<str.length;i++) n = (n*131 + str.charCodeAt(i)) >>> 0
    return n
  }
  const simulatedLiquidityEUR = useMemo(() => {
    const seed = seedFrom(todayISO())
    const base = totalReserve * (0.16 + ((seed % 7) * 0.01)) // ~16–22% of reserve
    const jitter = ((seed >> 8) % 1000) / 1000 * 0.06 // +0–6%
    return Math.round(base * (1 + jitter))
  }, [totalReserve])

  /* ===== Weekly per-category series (from asset histories) ===== */
  ...
  function openCategoryModal(catId) { setModalCat(catId); setOpenModal(true) }

  /* ===== Motion presets ===== */
  const fadeUp = { initial: { opacity: 0, y: 10 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true }, transition: { duration: .4 } }
      ...
      <Card className="p-0">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          ...
          <motion.div {...fadeUp} className="lg:col-span-7">
            <HeroDonut state={state} onOpenCategory={openCategoryModal} />
          </motion.div>

          {/* Right: KPIs + Category Explorer + CTA */}
          <motion.div {...fadeUp} className="lg:col-span-5 p-4 space-y-5">
